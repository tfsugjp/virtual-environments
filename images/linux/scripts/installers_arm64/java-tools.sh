...
createJavaEnvironmentalVariable() {
    ...
    case ${VENDOR_NAME} in
        ...
        "Adopt" )
            INSTALL_PATH_PATTERN="/usr/lib/jvm/adoptopenjdk-${JAVA_VERSION}-hotspot-aarch64" ;;

        "Temurin-Hotspot" )
            INSTALL_PATH_PATTERN="/usr/lib/jvm/temurin-${JAVA_VERSION}-jdk-aarch64" ;;
        *)
            echo "Unknown vendor"
            exit 1
...
installOpenJDK() {
    ...
    if [[ ${VENDOR_NAME} == "Temurin-Hotspot" ]]; then
        ...
        javaVersionPath="/usr/lib/jvm/temurin-${JAVA_VERSION}-jdk-aarch"
    elif [[ ${VENDOR_NAME} == "Adopt" ]]; then
        apt-get -y install adoptopenjdk-${JAVA_VERSION}-hotspot=\*
        javaVersionPath="/usr/lib/jvm/adoptopenjdk-${JAVA_VERSION}-hotspot-aarch"
    else
    ...
    touch "${javaToolcacheVersionPath}/aarch.complete"

    # Create symlink for Java
    ln -s ${javaVersionPath} "${javaToolcacheVersionPath}/aarch"
