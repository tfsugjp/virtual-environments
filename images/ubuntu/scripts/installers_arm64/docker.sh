...
echo "deb [arch=arm64 signed-by=$gpg_key] $repo_url $(getOSVersionLabel) stable" > $repo_path
apt-get update
...
URL=$(get_github_package_download_url "docker/compose" "contains(\"compose-linux-aarch64\")")
...
aws_helper_url=$(curl "${authString[@]}" -fsSL $aws_latest_release_url | jq -r '.body' | awk -F'[()]' '/linux-arm64/ {print $2}')
